...
public partial class Utilitys
{
    ...
    {
        string str_space = GetPadRightText(JobName, 30);
    ...
    public static void SetJobStepEnd(int jobStep, string jobName, int delay)
    {
        ...
        string str_start = StartTime.ToString("HH:mm:ss");   //取任務起始時間點
        string str_end = DateTime.Now.ToString("HH:mm:ss"); //取即時時間點
        string ls_delay = GetPadLeftText(delay.ToString(), 4);
        string ls_total = GetPadLeftText(Utilitys.TotalSeconds.ToString(), 4);
        JobMessage = string.Format("{0} {1} {2} {3} {4} {5}", str_seq, str_name, str_start, str_end, ls_delay, ls_total);
